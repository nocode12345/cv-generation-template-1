...
def generate_cv():
    ...
    try:
        ...
        # Ensure data is a dictionary (unwrap if it’s an array with a single object containing 'json', 'text', or 'cv_data')
        if isinstance(data, list) and len(data) == 1 and isinstance(data[0], dict):
            if 'json' in data[0]:  # Handle n8n's format
                data = data[0]['json']
            elif 'text' in data[0]:
                try:
            ...
            logger.error("Invalid JSON data structure: expected object or list with json/text/cv_data field")
            return jsonify({"error": "Invalid JSON data structure"}), 400

        # Remove fullRawText if it exists (using Python's in operator)
        if 'fullRawText' in data:
            del data['fullRawText']

        # Log the parsed JSON to debug
        logger.debug(f"Parsed JSON after unwrapping: {json.dumps(data, ensure_ascii=False)}")
